...
const { StatusModel, validStatus} = require('../models/statusModel');
...
router.get('/', async (req, res) => {
  ...
  router.get("/:travelerID", async (req, res) => {
    ...
    try {
        let data = await StatusModel.find({traveler_id:traveler_id_status})
        res.json(data);
    }
    catch (err) {
        console.log(err);
   ...
   /* Create a new status */
    ...
    let validBody = validStatus(req.body);
    if (validBody.error) {
        return res.status(400).json(validBody.error.details);
    }
    try {
        let status = new StatusModel(req.body);
...
router.put("/:statusEditID",  authToken, async (req, res) => {
    ...
    try {
        let itemEdit = await StatusModel.updateOne({ _id: id_status_edit, traveler_id: req.userData._id }, req.body);
        res.json(itemEdit);
    }
    catch (err) {
        console.log(err);
...
router.delete("/:statusDeleteID", authToken, async (req, res) => {
    ...
    try {
        let itemDelete = await StatusModel.deleteOne({ _id: deleteID /*, user_id: req.userData._id */  });
        res.json(itemDelete);
    }
    catch (err) {
        console.log(err);
